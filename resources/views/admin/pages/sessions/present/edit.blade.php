...
<div class="container-fluid">
    <div class="row mb-2">
      <div class="col-sm-6">
        <h1>Editar presença para  <strong>{{$session->nome}} {{$session->typeSession->nome}} - {{$session->legislature->descricao}}</strong></h1>
      </div>
      <div class="col-sm-6">
        <ol class="breadcrumb float-sm-right">
          <li class="breadcrumb-item"><a href="{{route('dashboard.index')}}">Dashbord</a></li>
          <li class="breadcrumb-item "> <a href="{{route('sessions.index')}}">Sessões </a></li>                   
          <li class="breadcrumb-item ">Editar Presença </li>          
        </ol>
      </div>
    </div>
  </div>
...
<div class="card">
    <div class="card-body">
        <form action="{{route('sessionPresentUpdate.update', $session->id)}}" method="POST">
            @csrf
            @method('PUT')

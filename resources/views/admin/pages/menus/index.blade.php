                        ...
                        <div class="col-md-2">
                            <div class="form-group">
                                ...
                                <select name="tipo_menu" class="form-control form-control-sm">
                                    ...
                                    <option value="simples" {{ request('tipo_menu') == 'simples' ? 'selected' : '' }}>Simples</option>
                                    <option value="dropdown" {{ request('tipo_menu') == 'dropdown' ? 'selected' : '' }}>Dropdown</option>
                                    <option value="mega_menu" {{ request('tipo_menu') == 'mega_menu' ? 'selected' : '' }}>Mega Menu</option>
                                    <option value="categoria" {{ request('tipo_menu') == 'categoria' ? 'selected' : '' }}>Categoria</option>
                                </select>
                            </div>
                        </div>
                        <div class="col-md-2">
                            <div class="form-group">
                                ...
                                <select name="posicao" class="form-control form-control-sm">
                                    ...
                                    <option value="1" {{ request('posicao') == '1' ? 'selected' : '' }}>Menu Principal</option>
                                    <option value="2" {{ request('posicao') == '2' ? 'selected' : '' }}>Barra Superior</option>
                        ...
                        <div class="col-md-3">
                            <div class="form-group">
                                ...
                                <div class="input-group input-group-sm">
                                    <input type="text" name="pesquisa" class="form-control" value="{{ request('pesquisa') }}" placeholder="Nome, URL, descrição...">
                                    ...
                                    <tr data-id="{{ $menu->id }}" class="menu-row menu-level-{{ $menu->level ?? 0 }}">
                                        ...
                                        <td>
                                            <div class="d-flex align-items-center">
                                                ...
                                                @if($menu->descricao)
                                                    <i class="fas fa-info-circle text-muted ml-1" title="{{ $menu->descricao }}"></i>
                                                @endif
                                            </div>
                                        </td>
                                        <td>
                                        ...
                                        <td class="text-center text-nowrap">
                                                ...
                                                @can('ver-menu')
                                                <a href="{{ route('admin.menus.show', $menu->id) }}" class="btn btn-flat  bg-gradient-info   " title="Visualizar">
                                                    <i class="fas fa-eye"></i>
                                                </a>
                                                @endcan
                                                <a href="{{ route('admin.menus.edit', $menu) }}" class="btn btn-flat  bg-gradient-warning " title="Editar">
                                                    <i class="fas fa-edit"></i>
                                                </a>
                                                <button type="button" class="btn btn-flat bg-gradient-secondary duplicate-btn" 
                                                        data-id="{{ $menu->id }}" data-nome="{{ $menu->nome }}" title="Duplicar">
        ...
        <div class="modal-dialog modal-xl">
            <div class="modal-content">
                <div class="modal-header">
                    <h4 class="modal-title"><i class="fas fa-eye"></i> Preview do Menu</h4>
...
/* Badges */
.badge-tipo-simples { background-color: #6c757d; }
.badge-tipo-dropdown { background-color: #ffc107; color: #212529; }
.badge-tipo-mega_menu { background-color: #28a745; }
.badge-tipo-categoria { background-color: #17a2b8; }
...
function initEventListeners() {
    ...
    $('#preview-btn').click(function() {
        const posicao = $('select[name="posicao"]').val() || 1;
        
        $('#preview-modal').modal('show');
        
        $.ajax({
            url: '{{ route("admin.menus.preview") }}',
            data: { posicao: posicao },
            success: function(response) {
                $('#preview-content').html(response.html);
            },
            error: function() {
                $('#preview-content').html('<div class="alert alert-danger">Erro ao carregar preview</div>');
            }
        });
    });
    ...
    $(document).on('click', '.duplicate-btn', function() {
        ...
        if (confirm(`Deseja duplicar o menu "${nome}"?`)) {
            $.ajax({

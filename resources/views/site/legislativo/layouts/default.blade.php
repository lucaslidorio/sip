...
<body class="bg-color-0">
  <div class="container-fluid   bg-color-1 bg-gradient  ps-5 pe-5 ">
    <div class=" justify-content-end d-none d-md-flex">
      <ul class="nav nav-pills fw-semibold font-14">
        <li class="nav-item ">
          <a href="#" id="diminuirFonte" class="nav-link link-texto pe-1 text-decoration-none "
            title="Diminuir o tamanho da fonte" data-bs-toggle="tooltip" data-bs-placement="bottom"
            data-bs-title="Diminuir o tamanho da fonte">
            <i class="bi bi-subscript"></i>
          </a>
        </li>
        <li class="nav-item ">
          <a href="#" id="resetarFonte" class="nav-link link-texto pe-2 ps-2 m-0" title="Fonte tamanho normal"
            data-bs-toggle="tooltip" data-bs-placement="bottom" data-bs-title="Fonte tamanho normal">
            <i class="bi bi-type"></i>
          </a>
        </li>
        <li class="nav-item ">
          <a href="#" id="aumentarFonte" class="nav-link link-texto ps-1 " title="Aumentar o tamanho da fonte"
            data-bs-toggle="tooltip" data-bs-placement="bottom" data-bs-title="Aumentar o tamanho da fonte"
        ...
        <li class="nav-item ">
          <a href="{{route('site.mapa')}}" class="nav-link  link-texto " title="Mapa do Site" data-bs-toggle="tooltip"
            data-bs-placement="bottom" data-bs-title="Mapa do Site">
        ...
        <li class="nav-item ">
          <a href="{{route('site.acessibilidade')}}" class="nav-link link-texto" title="Acessibilidade"
            data-bs-toggle="tooltip" data-bs-placement="bottom" data-bs-title="Acessibilidade">
            <i class="bi bi-universal-access-circle"></i>
            Acessibilidade
          </a>
        </li>
        <li class="nav-item ">
          <a href="{{route('pagina',  'contato')}}" class="nav-link link-texto" title="Contato" data-bs-toggle="tooltip"
            data-bs-placement="bottom" data-bs-title="Contato">
            <i class="bi bi-envelope-fill"></i>
            Contato
          </a>
        </li>
        <li class="nav-item ">
          <a href="#" id="contraste" class="nav-link link-texto" title="Contraste" data-bs-toggle="tooltip"
            data-bs-placement="bottom" data-bs-title="Contraste">
            <i class="bi bi-circle-half"></i>
            Contraste
          </a>
        </li>
        <li class="nav-item ">
          <a href="https://www.gov.br/governodigital/pt-br/vlibras/" class="nav-link link-texto" title="VLibras"
            data-bs-toggle="tooltip" data-bs-placement="bottom" data-bs-title="VLibras">
    ...
    <header class="d-flex flex-wrap justify-content-center py-2">
      ...
      <form class="col-12 col-lg-auto mb-3 mt-lg-4" action="{{ route('site.pesquisar') }}" method="get"
        role="pesquisar">
        @csrf
  ...
  <div class="d-flex justify-content-end bg-color-2 pe-5 mt-0 mb-0 ">
    ...
    <ul class="nav text-white fw-semibold font-14">
      ...
      <li class="nav-item ">
        ...
        <a href="{{route($item->url)}}" class="nav-link link-texto " title="{{$item->nome}} " data-bs-toggle="tooltip"
          data-bs-placement="top" data-bs-title="{{$item->nome}} ">
          <i class="{{$item->icone}}"></i>
          {{$item->nome}}
        </a>
        @else
        <a href="{{$item->url}}" target="{{$item->target ? '__blank': ''}}" class="nav-link link-texto "
          title="{{$item->nome}} " data-bs-toggle="tooltip" data-bs-placement="top" data-bs-title="{{$item->nome}} ">
          <i class="{{$item->icone}}"></i>
          {{$item->nome}}
        </a>
        @endif
        ...
        <div class="flex-shrink-0  bg-white " style="width: auto;">
          <ul class="list-unstyled ps-0">
            ...
            <li class="mb-1 bg-color-0">
              <button class=" w-100 btn btn-toggle  d-inline-flex align-items-center  border-0 collapsed bg-color-1"
                data-bs-toggle="collapse" data-bs-target="#{{$menu->slug}}" aria-expanded="false">
                {{$menu->nome}}
              </button>
              <div class="expand collapse show" id="{{$menu->slug}}">
                ...
                <div class="expand collapse show " id="{{$menu->slug}}">
                  <ul class="btn-toggle-nav  list-unstyled fw-normal pb-1  small ">
                    ...
                    @if ($item->pagina_interna == 1 && $item->url == null)
                    <li><a href="{{route('pagina', $item->slug)}}" target="{{$item->target ? '__blank': ''}}"
                        class="link-dark bg-color-0 d-flex text-decoration-none border-bottom p-2 mb-0 ">{{$item->nome}}</a>
                    </li>
                    @elseif($item->pagina_interna == 1 && $item->url != null)
                    <li><a href="{{route($item->url)}}" target="{{$item->target ? '__blank': ''}}"
                        class="link-dark bg-color-0 d-flex text-decoration-none border-bottom p-2 mb-0 ">{{$item->nome}}</a>
                    </li>
                    @else
                    <li><a href="{{$item->url}}" target="{{$item->target ? '__blank': ''}}"
                        class="link-dark bg-color-0 d-flex text-decoration-none border-bottom p-2 mb-0 ">{{$item->nome}}</a>
                    </li>
                    @endif
                    @endforeach
      ...
      <div class="col-md-3 col-lg-2  m-0 pt-md-0 p-md-3 p-sm-5  ">
        ...
        <div class="flex-shrink-0  bg-white " style="width: auto;">
          <ul class="list-unstyled ps-0">
            <li class="mb-1">
              <button class=" w-100 btn btn-toggle  d-inline-flex align-items-center  border-0 collapsed bg-color-1"
                data-bs-toggle="collapse" data-bs-target="#links-uteis" aria-expanded="false">
                <h5>Link Úteis</h5>
              </button>
                ...
                <ul class="btn-toggle-nav  list-unstyled fw-normal pb-1 small bg-color-0">
                  <li class="">
                    <a target="{{$link->target ? '_blank' : ''}}" href="{{$link->url}}"
                      class="link-dark bg-color-0 d-flex text-decoration-none border-bottom ">{{$link->nome}}</a>
                  </li>
                </ul>
              </div>
              @endforeach
  ...
  <div class="d-flex flex-column flex-sm-row justify-content-between bg-color-2 pe-5 ps-5 pt-1   border-top">
    ...
    <ul class="list-unstyled d-flex">
      @if($tenant->youtube)
      <li class="ms-3">
        <a class="link-dark" target="__blank" href="{{ $tenant->youtube }}">

        ...
        <div class="container">
            <div class="section-header">
                <h2 class="section-title wow fadeIn" data-wow-duration="1000ms" data-wow-delay="0.3s">
                    REGIMENTO INTERNO DO PODER <span>LEGISLATIVO </span>
                </h2>
            ...
            <div class="card">              
                <div class="card-body  p-0 table-bordered">
                    <table class="table table-hover">
                        ...
                        <tr>
                            <th scope="row">{{$legislacao->type_legislations->nome}}</th>
                            <td>{{$legislacao->numero}}</td>
                            <td>{{\Carbon\Carbon::parse($legislacao->data)->format('d/m/Y')}}</td>
                            {{-- <td> <a href="{{config('app.aws_url')."{$legislacao->legislacao}" }}" 
                ...
                <div class="card-footer" style="background:white;">
                    @if (!empty($filters))
                    {!!$legislacoes->appends($filters)->links()!!}
                    @else
                    {!!$legislacoes->links()!!}
                    @endif
                </div>

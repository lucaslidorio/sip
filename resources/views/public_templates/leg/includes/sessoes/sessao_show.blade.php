      ...
      <ul class="list-group list-group-flush">
          @forelse ($sessao->anexos as $anexo)
              <li class="list-group-item d-flex justify-content-between align-items-center">
                  <span>
                      <strong>{{ $anexo->typeDocument->nome ?? '-' }}</strong>  - {{ $anexo->descricao ?? $anexo->nome_original }}
                  </span>
                  <a href="{{config('app.aws_url')."{$anexo->anexo}" }}" target="_blank" class="btn btn-lg btn-outline-primary">Visualizar</a>
              </li>
          @empty
                    ...
                    @foreach ($propositurasVotadas as $prop)
                        <tr>
                            ...
                            <td class="text-center">
                                <a href="{{ route('camara.propositura.show', $prop->id) }}" class="btn btn-primary  cor-padrao-bg text-white btn-sm fs-4">Visualizar</a>
                            </td>
                        </tr>
                    @endforeach
                </tbody>
